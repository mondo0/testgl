...
from gl.compat import u
from gl.logger import logger
from gl.logs import glances_logs
...
class Screen(object):
    ...
    def display(self, stats, cs_status=None):
        ...
        if (gl_processes.max_processes is None or
                gl_processes.max_processes != max_processes_displayed):
        ...
        if self.edit_filter and cs_status is None:
            new_filter = self.display_popup(
                ...
                '- username:^root        ',
                is_input=True,
                input_value=gl_processes.process_filter_input)
            gl_processes.process_filter = new_filter
    ...
    def display_popup(self, message,
        ...
        if is_input:
            ...
            self.set_cursor(2)
            self.flash_cursor()
            textbox = Textbox(subpop, insert_mode=False)
            textbox.edit()
            self.set_cursor(0)
            self.no_flash_cursor()
            if textbox.gather() != '':
...
class Textbox(Textbox, object):
    ...
    def do_command(self, ch):
        ...
        if ch == 127:  # Back
            return 8
        return super(Textbox, self).do_command(ch)
